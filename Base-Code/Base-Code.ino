...
void modeLED();                                                              // colour for mode/heartbeat on Smart LED
void motorControl(int dir, int pwm, int in1, int in2);     
...
unsigned char driveStep;                                                      // state index for run mode
...
void loop()
{
   ...
   if ((currentMicros - previousMicros) >= 1000) {                             // enter when 1 ms has elapsed
      ...
      if (timerCount2sec > 6000) {                                             // if 2 seconds have elapsed
      ...
      switch(robotModeIndex) {
         case 0: // Robot stopped
            motorControl(0, 0, cIN1Chan[0], cIN2Chan[0]);                          // stop left motor
            motorControl(0, 0, cIN1Chan[1], cIN2Chan[1]);                          // stop right motor
            encoder[0].pos = 0;                                                // clear left encoder
            encoder[1].pos = 0;                                                // clear right encoder
            driveStep = 0;                                                    // reset drive index
            ...
            if (timeUp3sec) {                                                  // pause for 3 sec before running case 1 code
               ...
               if (motorsEnabled) {                                            // run motors only if enabled
                  if (timeUp2sec) {                                            // update drive state after 2 seconds
                     ...
                     switch(driveStep) {                                      // cycle through drive states
                        case 0: // Stop
                           motorControl(0, 0, cIN1Chan[0], cIN2Chan[0]);           // stop left motor
                           motorControl(0, 0, cIN1Chan[1], cIN2Chan[1]);           // stop right motor
                           driveStep = 1;                                     // next state: drive forward
                           break;

                        case 1: // Drive forward — motors spin in opposite directions as they are opposed by 180 degrees
                           motorControl(1, driveSpeed, cIN1Chan[0], cIN2Chan[0]);  // left motor forward
                           motorControl(-1, driveSpeed, cIN1Chan[1], cIN2Chan[1]); // right motor reverse (opposite dir from left)
                           driveStep = 2;                                     // next state: drive backward
                           break;

                        case 2: // Drive backward — motors spin in opposite directions as they are opposed by 180 degrees
                           motorControl(-1, driveSpeed, cIN1Chan[0], cIN2Chan[0]); // left motor reverse 
                           motorControl(1, driveSpeed, cIN1Chan[1], cIN2Chan[1]);  // right motor forward (opposite dir from right)
                           driveStep = 3;                                     // next state: turn left
                           break;

                        case 3: // Turn left (counterclockwise) - motors spin in same direction
                           motorControl(-1, driveSpeed, cIN1Chan[0], cIN2Chan[0]); // left motor reverse
                           motorControl(-1, driveSpeed, cIN1Chan[1], cIN2Chan[1]); // right motor reverse
                           driveStep = 4;                                     // next state: turn right
                           break;

                        case 4: // Turn right (clockwise) — motors spin in same direction
                           motorControl(1, driveSpeed, cIN1Chan[0], cIN2Chan[0]);  // left motor forward
                           motorControl(1, driveSpeed, cIN1Chan[1], cIN2Chan[1]);  // right motor forward
                           driveStep = 0;                                     // next state: stop
                           break;
                     }
                  }
               }
         ...
         case 2: //add your code to do something 
             if (timeUp3sec) {                                                  // pause for 3 sec before running case 1 code
                ...
                if (motorsEnabled) {   
               Serial.print(F("Left Encoder count = "));
               Serial.print(pos[0]); // print 
               Serial.print(F(" Right Encoder count = ")); // print position 
               Serial.println(pos[1]);   // Position                   
               switch(driveStep){
                    case 0:
                      motorControl(-1, driveSpeed, cIN1Chan[0], cIN2Chan[0]);  // left motor forward
                      motorControl(1, driveSpeed, cIN1Chan[1], cIN2Chan[1]); // right motor reverse (opposite dir from left)
                      if (encoder[0].pos > 10000){ // check left motor encoder postiton 
                      driveStep ++;
                      }
                      break;
                    case 1:
                      motorControl(1, driveSpeed, cIN1Chan[0], cIN2Chan[0]);  // left motor forward
                      motorControl(-1, driveSpeed, cIN1Chan[1], cIN2Chan[1]); // right motor reverse (opposite dir from left)
                      if (encoder[0].pos <5000){
                        driveStep ++;
                        timer = millis();
                      }
                      break;
                    case 2:
                      motorControl(0, driveSpeed, cIN1Chan[0], cIN2Chan[0]);  // left motor forward
                      motorControl(0, driveSpeed, cIN1Chan[1], cIN2Chan[1]); // right motor reverse (opposite dir from left)
