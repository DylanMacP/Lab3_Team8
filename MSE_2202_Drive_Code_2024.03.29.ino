...
#define SMART_LED           21
        // when DIP Switch S1-4 is on, Smart LED is connected to pin 23 GPIO21 (J21)
...
const int cLeftAdjust = 5;
        // Amount to slow down left motor relative to right
const int cRightAdjust = 0;
        // Amount to slow down right motor relative to left

const int TwoMeterApproximation = 13000; // Approximate Distance The Bot Goes Out Total

const int NinetyDegreeRotation = 1525; // Value for a 90 degree Rotation
      ...
      // read right encoder count
            ...
            switch(CollectionIndex) {
              ...
              case 1: //rotate 90 degrees 
                ...
                if (LeftEncoder.lRawEncoderCount >= NinetyDegreeRotation){
                  LeftEncoder.clearEncoder();
                  RightEncoder.clearEncoder();
                  CollectionIndex ++;
                }
              ...
              case 2: // Drive Forward Again 1 -> 2

                // move forward
                Bot.Forward("D1", leftDriveSpeed, rightDriveSpeed); //drive ID, left speed, right speed
                // move forward ~ 1m
                if (LeftEncoder.lRawEncoderCount >= TwoMeterApproximation/2){
                  LeftEncoder.clearEncoder();
                  RightEncoder.clearEncoder();
                  CollectionIndex ++;
                }
              ...
              case 4: // Drive Forward 2 -> 3
                ...
                break;
                 ...
                 case 7: //rotate parrallel to the collection area
                Bot.Left("D1", leftDriveSpeed, rightDriveSpeed); //drive ID, left speed, right speed
                // go back the same
                if (LeftEncoder.lRawEncoderCount >= (NinetyDegreeRotation)){
                  LeftEncoder.clearEncoder();
                ...
                case 8: // Drive Forward 6 -> Finish //THIS CASE WILL NEED EDITING UNLESS I GET THE VISION PERFECT
                ...
                if ((PreviousDistance - NewDistance)>=45){
                CollectionIndex ++;
                  }
                }
                RotationCounter++;
